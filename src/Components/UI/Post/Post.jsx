...
export default function Post({ post }) {
  ...
  return (
    <Grid container spacing={4} justifyItems="center" style={{ marginTop: '10px' }}>
      {allData.map((post) => {
        const { post_id, job_title, company_name, location, skills_required, description, created_at, name, teacher_id } =
          post;
          ...
          <Grid key={post_id} item xs={12} sm={6} md={4}>
            <Card sx={{ maxWidth: 400, marginTop: '20px', background: '#383838' }} className="a" variant="outlined">
              <CardHeader
                ...
                //   </Avatar>
                // }
                action={<MenuItems deletePost={deleteJobPost} postId={post_id} type={accountType} />}
              ...
              <Collapse in={expanded[post_id]} timeout="auto" unmountOnExit>
                <CardContent>
                  ...
                  <ul>
                    ...
                    <li>Good Knowledge of at least two popular Python framework (like Django, Tornado, Flask or Pyramid)</li>
